...
io.on('connection', function (socket) {
  ...
  socket.on('add user', function (username) {
    ...
    ++numUsers;
    addedUser = true;
    socket.emit('login', { //somewhere in our client there has to be socket.login
      numUsers: numUsers
    });
  ...
  socket.on('disconnect', function () {
    if (addedUser) {
      --numUsers; //decrease the amount of users

      // echo globally that this client has left
      socket.broadcast.emit('user left', {
        username: socket.username, //basically saying this blah blah user left 
        numUsers: numUsers
      });
    }
  });
